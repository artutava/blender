...
static bool kx_vertex_proxy_set_uvs_item_cb(void *self_v, int index, PyObject *item)
{
	...
	self->NotifyUpdate(RAS_IDisplayArray::UVS_MODIFIED);
...
static bool kx_vertex_proxy_set_colors_item_cb(void *self_v, int index, PyObject *item)
{
	...
	self->NotifyUpdate(RAS_IDisplayArray::COLORS_MODIFIED);
...
int KX_VertexProxy::pyattr_set_x(EXP_PyObjectPlus *self_v, const struct EXP_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	...
	if (PyFloat_Check(value)) {
		...
		self->NotifyUpdate(RAS_IDisplayArray::POSITION_MODIFIED);
...
int KX_VertexProxy::pyattr_set_normal(EXP_PyObjectPlus *self_v, const struct EXP_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	...
	if (PySequence_Check(value)) {
		...
		if (PyVecTo(value, vec)) {
			...
			self->NotifyUpdate(RAS_IDisplayArray::NORMAL_MODIFIED);
			return PY_SET_ATTR_SUCCESS;
		}
	}
	return PY_SET_ATTR_FAIL;
...
KX_VertexProxy::KX_VertexProxy(CM_UpdateServer<RAS_IDisplayArray> *updateServer, RAS_Vertex vertex)
	:m_vertex(vertex),
...
void KX_VertexProxy::NotifyUpdate(unsigned int flag)
{
	if (m_updateServer) {
		m_updateServer->NotifyUpdate(flag);
	}
}
...
PyObject *KX_VertexProxy::PySetXYZ(PyObject *value)
{
	...
	m_vertex.SetXYZ(vec);
	NotifyUpdate(RAS_IDisplayArray::POSITION_MODIFIED);
	Py_RETURN_NONE;
}
...
PyObject *KX_VertexProxy::PySetNormal(PyObject *value)
{
	...
	NotifyUpdate(RAS_IDisplayArray::NORMAL_MODIFIED);
	Py_RETURN_NONE;
}
...
PyObject *KX_VertexProxy::PySetRGBA(PyObject *value)
{
	if (PyLong_Check(value)) {
		...
		NotifyUpdate(RAS_IDisplayArray::COLORS_MODIFIED);
		Py_RETURN_NONE;
	}
	else {
		mt::vec4 vec;
...
PyObject *KX_VertexProxy::PySetUV1(PyObject *value)
{
	...
	NotifyUpdate(RAS_IDisplayArray::UVS_MODIFIED);
	Py_RETURN_NONE;
}
